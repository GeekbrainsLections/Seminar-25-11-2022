...
Для cкачиваниz репозитория из GitHub используется команда *git clone*. Для этого в терминале с открытой папкой-репозиторием необходимо написать *git clone <ссылка_на_чужой_репозиторий>*. Далее написать *cd <название_скачанной_папки>*.
